...
export type Moment = MomentDef & { ready: boolean; src: string; width: number; height: number };
...
function load(m: MomentDef): Moment {
  const size = pngSize(path.join(process.cwd(), "public", SCREENS_DIR, m.file));
  return { ...m, ready: !!size, src: `/${SCREENS_DIR}/${m.file}`, width: size?.width ?? 1548, height: size?.height ?? 500 };
}
