...
export function Capture({ moment, priority = false }: { moment: Moment; priority?: boolean }) {
    ...
    return (
      <figure className="hw-capture hw-capture--pending">
        <div className="hw-capture-pending">
          ...
          <strong>Inside the real workspace</strong>
  ...
  return (
    <>
      <figure className="hw-capture">
        <button type="button" className="hw-capture-open" onClick={() => dialog.current?.showModal()} aria-label={`Enlarge the capture: ${moment.tabTitle}`}>
          ...
          />
          <span className="hw-capture-zoom" aria-hidden="true">Enlarge ⤢</span>
        </button>
      </figure>
      <noscript>
        <a className="hw-capture-enlarge" href={moment.src}>Open the full-size capture</a>
      </noscript>
      <dialog
        ref={dialog}
        className="hw-zoom"
        aria-label={`Full-size capture: ${moment.tabTitle}`}
        onClick={(e) => { if (e.target === dialog.current) dialog.current?.close(); }}
      >
        <div className="hw-zoom-bar">
          <span>{moment.caption}</span>
          <button type="button" onClick={() => dialog.current?.close()}>Close <span aria-hidden="true">✕</span></button>
        </div>
        <div className="hw-zoom-scroll" tabIndex={0} aria-label="Scrollable capture">
          {/* eslint-disable-next-line @next/next/no-img-element */}
